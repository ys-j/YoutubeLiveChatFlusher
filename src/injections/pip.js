(function initPipMenu() {
	 ...
	 * @param {DOMStringMap} params dataset of the current script
	...
	async function openPip(element, params) {
		...
		const { logger } = await import('../modules/logging.mjs');
		const nonce = params?.nonce;

		/** @type {Record<string, { key: string, alt: boolean }>} */
		const userDefinedHotkeys = JSON.parse(params?.hotkeys || '{}');
