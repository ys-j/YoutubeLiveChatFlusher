...
export class LiveChatPanel {
	...
	constructor(div = undefined) {
		...
		fields[0].innerHTML = [
			...
			`<div><div>${$msg('commonCSS')}</div><div><input type="text" name="_css" placeholder="${$msg('placeholder_customCSS')}" value="${escapeHtml(g.storage.cssTexts['div'])}" style="width:20.5em"></div></div>`,
		...
		if (elem.tagName === 'SELECT' || elem instanceof HTMLSelectElement) {
			if (name in g.storage.others) {
				...
				if (mode === g.index.mutedWords.char) {
					replacement.title = $msg('tooltip_mutedWordsReplacement');
				} else {
